...
from play_annotation_generator.field_goal_block_naming_migration import apply_plan, build_plan
...
def _fixture(tmp_path: Path, ids: tuple[int, ...] = (1, 2)):
    ...
    source_folder = tracking_root / "FieldGoalBlocks"
    source_folder.mkdir(parents=True)
    for clip_id in ids:
        (source_folder / f"FieldGoalBlocks_{clip_id}_cvat_mot.zip").write_bytes(f"zip-{clip_id}".encode())

    key_root = tmp_path / "key_actions"
    key_root.mkdir()
    (key_root / "FieldGoalBlock.csv").write_text("Video Name:,Field Goal Block\r\n", encoding="utf-8")

    summary = tmp_path / "DatasetSummary.csv"
    _write_summary(summary, [f"FieldGoalBlocks_{clip_id}.mp4" for clip_id in ids])
...
def test_build_plan_pairs_tracking_and_summary_ids(tmp_path):
    ...
    assert [p.tracking_target.name for p in plan.pairs] == [
        "FieldGoalBlock_1_cvat_mot.zip",
        "FieldGoalBlock_3_cvat_mot.zip",
    ]
    assert [p.summary_target for p in plan.pairs] == ["FieldGoalBlock_1.mp4", "FieldGoalBlock_3.mp4"]


def test_dry_run_planning_does_not_modify_files(tmp_path):
    tracking_root, key_root, summary = _fixture(tmp_path)
    before = summary.read_bytes()
    key_before = (key_root / "FieldGoalBlock.csv").read_bytes()
    plan = build_plan(tracking_root, summary, key_root)
    assert plan.ready
    assert summary.read_bytes() == before
    assert (key_root / "FieldGoalBlock.csv").read_bytes() == key_before
    assert (tracking_root / "FieldGoalBlocks").is_dir()
    assert not (tracking_root / "FieldGoalBlock").exists()
...
def test_apply_updates_tracking_and_summary_but_not_key_actions(tmp_path):
    ...
    assert not (tracking_root / "FieldGoalBlocks").exists()
    assert (tracking_root / "FieldGoalBlock" / "FieldGoalBlock_1_cvat_mot.zip").read_bytes() == b"zip-1"
    assert (tracking_root / "FieldGoalBlock" / "FieldGoalBlock_2_cvat_mot.zip").read_bytes() == b"zip-2"
    assert (key_root / "FieldGoalBlock.csv").read_bytes() == key_before
    text = summary.read_text(encoding="utf-8")
    assert "FieldGoalBlock_1.mp4" in text and "FieldGoalBlock_2.mp4" in text
    assert "FieldGoalBlocks_" not in text


def test_tracking_folder_collision_blocks_plan(tmp_path):
    tracking_root, key_root, summary = _fixture(tmp_path)
    (tracking_root / "FieldGoalBlock").mkdir()
    plan = build_plan(tracking_root, summary, key_root)
...
def test_missing_canonical_key_actions_blocks_plan(tmp_path):
    tracking_root, key_root, summary = _fixture(tmp_path)
    (key_root / "FieldGoalBlock.csv").unlink()
    plan = build_plan(tracking_root, summary, key_root)
    assert not plan.ready
...
def test_id_mismatch_blocks_plan(tmp_path):
    ...
    _write_summary(summary, ["FieldGoalBlocks_1.mp4"])
    plan = build_plan(tracking_root, summary, key_root)
    assert not plan.ready
...
def test_summary_target_collision_blocks_plan(tmp_path):
    ...
    _write_summary(summary, ["FieldGoalBlocks_1.mp4", "FieldGoalBlock_1.mp4"])
    plan = build_plan(tracking_root, summary, key_root)
    assert not plan.ready
...
def test_unexpected_tracking_entry_blocks_plan(tmp_path):
    tracking_root, key_root, summary = _fixture(tmp_path)
    (tracking_root / "FieldGoalBlocks" / "notes.txt").write_text("unexpected", encoding="utf-8")
    plan = build_plan(tracking_root, summary, key_root)
    assert not plan.ready
