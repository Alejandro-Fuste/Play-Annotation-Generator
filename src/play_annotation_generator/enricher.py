...
def _partition_match_issues(
    match_issues: List[str],
    allow_unresolved_position_targets: bool = False,
) -> Tuple[List[str], List[str]]:
    """
    Split action-target resolution issues into warnings and errors.

    Missing position/team-side information is downgraded to a warning only when
    the caller explicitly indicates that player-track assignments are unavailable.
    When assignment data is expected to exist, unresolved group/position targets
    remain validation errors so incomplete or incorrect position mappings are not
    silently weakened.
    """
    warnings: List[str] = []
    errors: List[str] = []
    ...
    for issue in match_issues:
        if allow_unresolved_position_targets and "position/team_side missing" in issue:
...
def run_enrichment_pipeline(
    ...
    Args:
        xml_path: Path to the CVAT XML input.
        csv_path: Path to the wide-format Key Actions CSV.
        config: Pipeline configuration.
        target_video: Optional video filter.
        allow_unresolved_position_targets: If True, group/position target
            resolution failures caused specifically by missing position/team-side
            information are warnings instead of errors. This should only be True
            when player-track assignment data is intentionally unavailable.
    ...
    match_warnings, match_errors = _partition_match_issues(
        match_issues,
        allow_unresolved_position_targets=allow_unresolved_position_targets,
    )
