...
LEGACY_FOLDER = "Sacks"
LEGACY_NAME = "sack"
LEGACY_KEY_ACTIONS = "Sacks.csv"
CANONICAL_NAME = "Sack"
TRACKING_PATTERN = re.compile(r"^sack_(?P<id>\d+)_cvat_mot\.zip$")
SUMMARY_PATTERN = re.compile(r"^sack_(?P<id>\d+)\.mp4$")
...
def build_plan(tracking_root: Path, dataset_summary: Path, key_actions_root: Path) -> MigrationPlan:
    ...
    key_source = key_actions_root / LEGACY_KEY_ACTIONS
    ...
    if not source_folder.is_dir():
        errors.append(f"[Sack] legacy tracking folder not found: {source_folder}")
    if target_folder.exists():
        errors.append(f"[Sack] canonical tracking folder already exists: {target_folder}")
    if not key_source.is_file():
        errors.append(f"[Sack] legacy Key Actions file not found: {key_source}")
    if key_target.exists():
        errors.append(f"[Sack] canonical Key Actions target already exists: {key_target}")
    if errors:
        return MigrationPlan(source_folder, target_folder, key_source, key_target, (), tuple(errors))
    ...
    for path in sorted(source_folder.iterdir(), key=lambda p: p.name):
        ...
        if clip_id in tracking_by_id:
            errors.append(f"[Sack] duplicate legacy tracking ID {clip_id}")
        else:
            tracking_by_id[clip_id] = path
    if unexpected:
        errors.append(f"[Sack] unexpected entries in legacy tracking folder: {unexpected}")
    ...
    for row_number, output_file in summary_rows:
        ...
        if clip_id in summary_by_id:
            errors.append(f"[Sack] duplicate legacy DatasetSummary ID {clip_id}")
        else:
    ...
    if not tracking_ids:
        errors.append("[Sack] no legacy sack tracking files were found")
    if not summary_ids:
        errors.append("[Sack] no legacy sack DatasetSummary rows were found")

    if tracking_ids != summary_ids:
        only_tracking = sorted(tracking_ids - summary_ids)
        only_summary = sorted(summary_ids - tracking_ids)
        if only_tracking:
            errors.append(f"[Sack] IDs present in tracking but missing from DatasetSummary: {only_tracking}")
        if only_summary:
            errors.append(f"[Sack] IDs present in DatasetSummary but missing from tracking: {only_summary}")
    ...
    for clip_id in sorted(tracking_ids & summary_ids):
        ...
        if all_outputs.get(new_output):
            errors.append(
                f"[Sack] DatasetSummary target collision for ID {clip_id}: "
...
def _render_updated_summary(dataset_summary: Path, pairs: Iterable[ClipPair]) -> bytes:
    ...
    for pair in pairs:
        ...
        if body.count(pair.summary_source) != 1:
            raise ValueError(
                f"DatasetSummary row {pair.summary_row} does not contain exactly one occurrence "
                f"of {pair.summary_source!r}; refusing a non-surgical rewrite"
            )
...
def apply_plan(plan: MigrationPlan, dataset_summary: Path) -> None:
    ...
    fd, temp_name = tempfile.mkstemp(
        prefix=f".{dataset_summary.name}.sack.", suffix=".tmp", dir=str(dataset_summary.parent)
    )
    os.close(fd)
...
def print_plan(plan: MigrationPlan) -> None:
    print("Sack canonical naming migration")
    print("=" * 31)
    if plan.errors:
        print("\nVALIDATION ERRORS")
    ...
    for pair in plan.pairs:
        print(f"  [Sack #{pair.clip_id}, row {pair.summary_row}]")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Plan or apply the Sacks/sack -> Sack canonical naming migration. Dry-run is default.")
    ...
    try:
        ...
        return 1
    print("\nAPPLY COMPLETE: Sack tracking paths, Key Actions source, and DatasetSummary were updated.")
    return 0
