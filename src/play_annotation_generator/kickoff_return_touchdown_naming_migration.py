...
LEGACY_FOLDER = "KickoffReturnTDs"
LEGACY_NAME = "KickoffReturnTD"
CANONICAL_NAME = "KickoffReturnTouchdown"
TRACKING_PATTERN = re.compile(r"^KickoffReturnTD_(?P<id>\d+)_cvat_mot\.zip$")
SUMMARY_PATTERN = re.compile(r"^KickoffReturnTD_(?P<id>\d+)\.mp4$")
...
def build_plan(tracking_root: Path, dataset_summary: Path, key_actions_root: Path) -> MigrationPlan:
    ...
    key_source = key_actions_root / f"{LEGACY_NAME}.csv"
    key_target = key_actions_root / f"{CANONICAL_NAME}.csv"
    errors: list[str] = []
    ...
    if errors:
        return MigrationPlan(source_folder, target_folder, key_source, key_target, (), tuple(errors))

    if not source_folder.is_dir():
        errors.append(f"[KickoffReturnTouchdown] legacy tracking folder not found: {source_folder}")
    if target_folder.exists():
        errors.append(f"[KickoffReturnTouchdown] canonical tracking folder already exists: {target_folder}")
    if not key_source.is_file():
        errors.append(f"[KickoffReturnTouchdown] legacy Key Actions file not found: {key_source}")
    if key_target.exists():
        errors.append(f"[KickoffReturnTouchdown] canonical Key Actions target already exists: {key_target}")
    if errors:
        return MigrationPlan(source_folder, target_folder, key_source, key_target, (), tuple(errors))
    ...
    for path in sorted(source_folder.iterdir(), key=lambda p: p.name):
        ...
        if clip_id in tracking_by_id:
            errors.append(f"[KickoffReturnTouchdown] duplicate legacy tracking ID {clip_id}")
        else:
            tracking_by_id[clip_id] = path
    if unexpected:
        errors.append(
            f"[KickoffReturnTouchdown] unexpected entries in legacy tracking folder: {unexpected}"
        )
    ...
    for row_number, output_file in summary_rows:
        ...
        if clip_id in summary_by_id:
            errors.append(f"[KickoffReturnTouchdown] duplicate legacy DatasetSummary ID {clip_id}")
        else:
    ...
    if not tracking_ids:
        errors.append("[KickoffReturnTouchdown] no legacy KickoffReturnTD tracking files were found")
    if not summary_ids:
        errors.append("[KickoffReturnTouchdown] no legacy KickoffReturnTD DatasetSummary rows were found")
    ...
    if tracking_ids != summary_ids:
        ...
        if only_tracking:
            errors.append(
                "[KickoffReturnTouchdown] IDs present in tracking but missing from DatasetSummary: "
                f"{only_tracking}"
            )
        if only_summary:
            errors.append(
                "[KickoffReturnTouchdown] IDs present in DatasetSummary but missing from tracking: "
                f"{only_summary}"
            )
    ...
    for clip_id in sorted(tracking_ids & summary_ids):
        ...
        if all_outputs.get(new_output):
            errors.append(
                f"[KickoffReturnTouchdown] DatasetSummary target collision for ID {clip_id}: "
        ...
        pairs.append(
            ClipPair(
                ...
                summary_source=old_output,
                summary_target=new_output,
            )
        )
...
def _render_updated_summary(dataset_summary: Path, pairs: Iterable[ClipPair]) -> bytes:
    ...
    for pair in pairs:
        ...
        if current != pair.summary_source:
            raise ValueError(
                f"DatasetSummary row {pair.summary_row} changed since planning: "
                f"expected {pair.summary_source!r}, found {current!r}"
            )
        if body.count(pair.summary_source) != 1:
            raise ValueError(
                f"DatasetSummary row {pair.summary_row} does not contain exactly one "
                f"occurrence of {pair.summary_source!r}; refusing a non-surgical rewrite"
            )
        lines[line_index] = body.replace(pair.summary_source, pair.summary_target, 1) + ending
...
def apply_plan(plan: MigrationPlan, dataset_summary: Path) -> None:
    ...
    fd, temp_name = tempfile.mkstemp(
        prefix=f".{dataset_summary.name}.kickoff-return-touchdown.",
        suffix=".tmp",
    ...
    try:
        ...
        if plan.target_folder.exists():
            raise FileExistsError(
                f"Canonical tracking folder appeared before apply: {plan.target_folder}"
            )
        if plan.key_actions_target.exists():
            raise FileExistsError(
                f"Canonical Key Actions file appeared before apply: {plan.key_actions_target}"
            )
        ...
        for pair in plan.pairs:
            if not pair.tracking_source.exists():
                raise FileNotFoundError(
                    f"Tracking source disappeared before apply: {pair.tracking_source}"
                )
            if pair.tracking_target.exists():
                raise FileExistsError(
                    f"Tracking target appeared before apply: {pair.tracking_target}"
                )
        ...
        if any(plan.source_folder.iterdir()):
            raise RuntimeError(
                f"Legacy tracking folder is not empty after planned renames: {plan.source_folder}"
            )
        ...
        try:
            if source_folder_removed and not plan.source_folder.exists():
        ...
        try:
            if target_folder_created and plan.target_folder.exists() and not any(plan.target_folder.iterdir()):
        ...
        if rollback_errors:
            raise RuntimeError(
                "Migration failed and rollback was incomplete: " + "; ".join(rollback_errors)
            )
        raise
    finally:
        if temp_path.exists():
...
def print_plan(plan: MigrationPlan) -> None:
    print("KickoffReturnTouchdown canonical naming migration")
    print("=" * 49)
    ...
    print(f"Key Actions: {plan.key_actions_source} -> {plan.key_actions_target}")
    print(f"\nTracking + DatasetSummary pairs ({len(plan.pairs)}):")
    for pair in plan.pairs:
        print(f"  [KickoffReturnTouchdown #{pair.clip_id}, row {pair.summary_row}]")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Plan or apply the KickoffReturnTD -> KickoffReturnTouchdown canonical "
            "naming migration. Dry-run is the default."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "\nAPPLY COMPLETE: KickoffReturnTouchdown tracking paths, Key Actions source, "
        "and DatasetSummary were updated."
    )
    return 0
