...
LEGACY_NAME = "Hitch9to15"
CANONICAL_NAME = "Hitch"
TRACKING_PATTERN = re.compile(r"^Hitch9to15_(?P<id>\d+)_cvat_mot\.zip$")
SUMMARY_PATTERN = re.compile(r"^Hitch9to15_(?P<id>\d+)\.mp4$")
...
def _scan_summary(path: Path) -> tuple[list[tuple[int, str]], dict[str, list[int]], int]:
    raw = path.read_bytes()
    ...
    return rows, outputs, output_index
...
def build_plan(tracking_root: Path, dataset_summary: Path, key_actions_root: Path) -> MigrationPlan:
    ...
    if not source_folder.is_dir():
        errors.append(f"[Hitch] legacy tracking folder not found: {source_folder}")
    if target_folder.exists():
        errors.append(f"[Hitch] canonical tracking folder already exists: {target_folder}")
    if not key_source.is_file():
        errors.append(f"[Hitch] legacy Key Actions file not found: {key_source}")
    if key_target.exists():
        errors.append(f"[Hitch] canonical Key Actions target already exists: {key_target}")
    if errors:
        return MigrationPlan(source_folder, target_folder, key_source, key_target, (), tuple(errors))
    ...
    for path in sorted(source_folder.iterdir(), key=lambda p: p.name):
        ...
        if clip_id in tracking_by_id:
            errors.append(f"[Hitch] duplicate legacy tracking ID {clip_id}")
        else:
            tracking_by_id[clip_id] = path
    if unexpected:
        errors.append(f"[Hitch] unexpected entries in legacy tracking folder: {unexpected}")

    try:
        summary_rows, all_outputs, _ = _scan_summary(dataset_summary)
    ...
    for row_number, output_file in summary_rows:
        ...
        if clip_id in summary_by_id:
            errors.append(f"[Hitch] duplicate legacy DatasetSummary ID {clip_id}")
        else:
    ...
    if not tracking_ids:
        errors.append("[Hitch] no legacy Hitch9to15 tracking files were found")
    if not summary_ids:
        errors.append("[Hitch] no legacy Hitch9to15 DatasetSummary rows were found")
    if tracking_ids != summary_ids:
        only_tracking = sorted(tracking_ids - summary_ids)
        only_summary = sorted(summary_ids - tracking_ids)
        if only_tracking:
            errors.append(f"[Hitch] IDs present in tracking but missing from DatasetSummary: {only_tracking}")
        if only_summary:
            errors.append(f"[Hitch] IDs present in DatasetSummary but missing from tracking: {only_summary}")

    pairs: list[ClipPair] = []
    for clip_id in sorted(tracking_ids & summary_ids):
        source = tracking_by_id[clip_id]
        target = target_folder / f"Hitch_{clip_id}_cvat_mot.zip"
        row_number, old_output = summary_by_id[clip_id]
        new_output = f"Hitch_{clip_id}.mp4"
        if all_outputs.get(new_output):
            errors.append(
                f"[Hitch] DatasetSummary target collision for ID {clip_id}: "
    ...
    for pair in pairs:
        ...
        if body.count(pair.summary_source) != 1:
            raise ValueError(
                ...
                f"occurrence of {pair.summary_source!r}"
            )
...
def apply_plan(plan: MigrationPlan, dataset_summary: Path) -> None:
    ...
    fd, temp_name = tempfile.mkstemp(
        prefix=f".{dataset_summary.name}.hitch.", suffix=".tmp", dir=str(dataset_summary.parent)
    )
    os.close(fd)
    ...
    try:
        ...
        if folder_created and plan.target_folder.exists() and not any(plan.target_folder.iterdir()):
...
def print_plan(plan: MigrationPlan) -> None:
    print("Hitch canonical naming migration")
    print("=" * 31)
    if plan.errors:
        print("\nVALIDATION ERRORS")
    ...
    for pair in plan.pairs:
        print(f"  [Hitch #{pair.clip_id}, row {pair.summary_row}]")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Plan or apply the Hitch9to15 -> Hitch canonical naming migration. Dry-run is the default.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 1
    print("\nAPPLY COMPLETE: Hitch tracking paths, Key Actions source, and DatasetSummary were updated.")
    return 0
