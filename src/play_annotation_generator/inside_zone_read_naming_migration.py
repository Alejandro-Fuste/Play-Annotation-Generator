...
LEGACY_FOLDER = "InsideZone"
LEGACY_NAME = "InsideZoneOpenFormations"
CANONICAL_NAME = "InsideZoneRead"
LEGACY_KEY_ACTIONS_NAME = "InsideZone"
TRACKING_PATTERN = re.compile(r"^InsideZoneOpenFormations_(?P<id>\d+)_cvat_mot\.zip$")
SUMMARY_PATTERN = re.compile(r"^InsideZoneOpenFormations_(?P<id>\d+)\.mp4$")
EXPECTED_LIVE_PAIRED_COUNT = 234
...
def build_plan(
    ...
    dataset_summary: Path | str,
    key_actions_root: Path | str,
    ...
    key_actions_source = key_actions_root / f"{LEGACY_KEY_ACTIONS_NAME}.csv"
    key_actions_target = key_actions_root / f"{CANONICAL_NAME}.csv"
    errors: list[str] = []
    ...
    if errors:
        return MigrationPlan(source_folder, target_folder, key_actions_source, key_actions_target, (), tuple(errors))

    if not source_folder.is_dir():
        errors.append(f"[InsideZoneRead] legacy tracking folder not found: {source_folder}")
    if target_folder.exists():
        errors.append(f"[InsideZoneRead] canonical tracking folder already exists: {target_folder}")
    if not key_actions_source.is_file():
        errors.append(f"[InsideZoneRead] legacy Key Actions file not found: {key_actions_source}")
    if key_actions_target.exists():
        errors.append(f"[InsideZoneRead] canonical Key Actions target already exists: {key_actions_target}")
    if errors:
        return MigrationPlan(source_folder, target_folder, key_actions_source, key_actions_target, (), tuple(errors))
    ...
    for path in sorted(source_folder.iterdir(), key=lambda p: p.name):
        ...
        if clip_id in tracking_by_id:
            errors.append(f"[InsideZoneRead] duplicate legacy tracking ID {clip_id}")
        else:
            tracking_by_id[clip_id] = path
    if unexpected:
        errors.append(f"[InsideZoneRead] unexpected entries in legacy tracking folder: {unexpected}")
    ...
    for row_number, output_file in summary_rows:
        ...
        if clip_id in summary_by_id:
            errors.append(f"[InsideZoneRead] duplicate legacy DatasetSummary ID {clip_id}")
        else:
    ...
    if not tracking_ids:
        errors.append("[InsideZoneRead] no legacy InsideZoneOpenFormations tracking files were found")
    if not summary_ids:
        errors.append("[InsideZoneRead] no legacy InsideZoneOpenFormations DatasetSummary rows were found")

    if tracking_ids != summary_ids:
        only_tracking = sorted(tracking_ids - summary_ids)
        only_summary = sorted(summary_ids - tracking_ids)
        if only_tracking:
            errors.append(f"[InsideZoneRead] IDs present in tracking but missing from DatasetSummary: {only_tracking}")
        if only_summary:
            errors.append(f"[InsideZoneRead] IDs present in DatasetSummary but missing from tracking: {only_summary}")
    ...
    for clip_id in sorted(tracking_ids & summary_ids):
        ...
        if all_outputs.get(new_output):
            errors.append(
                f"[InsideZoneRead] DatasetSummary target collision for ID {clip_id}: "
    ...
    if expected_count is not None and len(pairs) != expected_count:
        errors.append(f"[InsideZoneRead] expected exactly {expected_count} paired clips, found {len(pairs)}")

    return MigrationPlan(source_folder, target_folder, key_actions_source, key_actions_target, tuple(pairs), tuple(errors))
...
def apply_plan(plan: MigrationPlan, dataset_summary: Path | str) -> None:
    if not plan.ready:
    ...
    fd, temp_name = tempfile.mkstemp(
        prefix=f".{dataset_summary.name}.inside-zone-read.", suffix=".tmp", dir=str(dataset_summary.parent)
    )
    os.close(fd)
    ...
    try:
        ...
        if not plan.key_actions_source.is_file():
            raise FileNotFoundError(f"Legacy Key Actions file disappeared before apply: {plan.key_actions_source}")
        if plan.key_actions_target.exists():
            raise FileExistsError(f"Canonical Key Actions target appeared before apply: {plan.key_actions_target}")
        ...
        try:
            if key_actions_renamed and plan.key_actions_target.exists() and not plan.key_actions_source.exists():
...
def print_plan(plan: MigrationPlan) -> None:
    print("InsideZoneRead canonical naming migration")
    print("=" * 44)
    print(f"Status: {'READY' if plan.ready else 'BLOCKED'}")
    print(f"paired clips: {len(plan.pairs)}")
    if plan.errors:
        print("\nVALIDATION ERRORS")
        for error in plan.errors:
            print(f"  - {error}")
    print("\nTracking folder:")
    print(f"    {plan.source_folder}")
    print(f"        -> {plan.target_folder}")
    print("\nKey Actions file:")
    print(f"    {plan.key_actions_source}")
    print(f"        -> {plan.key_actions_target}")
    print(f"\nTracking + DatasetSummary pairs ({len(plan.pairs)}):")
    for pair in plan.pairs:
        print(f"  [InsideZoneRead #{pair.clip_id}, row {pair.summary_row}]")
        print(f"    {pair.tracking_source.name}")
        print(f"        -> {pair.tracking_target.name}")
        print(f"    {pair.summary_source}")
        print(f"        -> {pair.summary_target}")
    print(f"\nStatus: {'READY' if plan.ready else 'BLOCKED'} | paired clips: {len(plan.pairs)}")


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Plan or apply the InsideZoneOpenFormations -> InsideZoneRead canonical naming migration. Dry-run is default."
    )
    ...
    parser.add_argument("--expected-count", type=int, default=EXPECTED_LIVE_PAIRED_COUNT)
    ...
    try:
        ...
        return 1
    print("\nAPPLY COMPLETE: InsideZoneRead tracking identities, Key Actions file, and DatasetSummary were updated.")
    return 0
