...
LEGACY_NAME = "Crosses25+"
CANONICAL_NAME = "DeepCross"
TRACKING_PATTERN = re.compile(r"^Crosses25\+_(?P<id>\d+)_cvat_mot\.zip$")
SUMMARY_PATTERN = re.compile(r"^Crosses25\+_(?P<id>\d+)\.mp4$")
...
def build_plan(tracking_root: Path, dataset_summary: Path, key_actions_root: Path) -> MigrationPlan:
    ...
    source_folder = tracking_root / LEGACY_NAME
    ...
    if not source_folder.is_dir():
        errors.append(f"[DeepCross] legacy tracking folder not found: {source_folder}")
    if target_folder.exists():
        errors.append(f"[DeepCross] canonical tracking folder already exists: {target_folder}")
    if not key_source.is_file():
        errors.append(f"[DeepCross] legacy Key Actions file not found: {key_source}")
    if key_target.exists():
        errors.append(f"[DeepCross] canonical Key Actions target already exists: {key_target}")
    if errors:
        return MigrationPlan(source_folder, target_folder, key_source, key_target, (), tuple(errors))
    ...
    for path in sorted(source_folder.iterdir(), key=lambda p: p.name):
        ...
        if clip_id in tracking_by_id:
            errors.append(f"[DeepCross] duplicate legacy tracking ID {clip_id}")
        else:
            tracking_by_id[clip_id] = path
    if unexpected:
        errors.append(f"[DeepCross] unexpected entries in legacy tracking folder: {unexpected}")
    ...
    for row_number, output_file in summary_rows:
        ...
        if clip_id in summary_by_id:
            errors.append(f"[DeepCross] duplicate legacy DatasetSummary ID {clip_id}")
        else:
    ...
    if not tracking_ids:
        errors.append("[DeepCross] no legacy Crosses25+ tracking files were found")
    if not summary_ids:
        errors.append("[DeepCross] no legacy Crosses25+ DatasetSummary rows were found")

    if tracking_ids != summary_ids:
        only_tracking = sorted(tracking_ids - summary_ids)
        only_summary = sorted(summary_ids - tracking_ids)
        if only_tracking:
            errors.append(f"[DeepCross] IDs present in tracking but missing from DatasetSummary: {only_tracking}")
        if only_summary:
            errors.append(f"[DeepCross] IDs present in DatasetSummary but missing from tracking: {only_summary}")
    ...
    for clip_id in sorted(tracking_ids & summary_ids):
        ...
        if all_outputs.get(new_output):
            errors.append(
                f"[DeepCross] DatasetSummary target collision for ID {clip_id}: "
    ...
    return MigrationPlan(source_folder, target_folder, key_source, key_target, tuple(pairs), tuple(errors))
...
def apply_plan(plan: MigrationPlan, dataset_summary: Path) -> None:
    ...
    fd, temp_name = tempfile.mkstemp(
        prefix=f".{dataset_summary.name}.deepcross.", suffix=".tmp", dir=str(dataset_summary.parent)
    )
    os.close(fd)
    ...
    try:
        ...
        if plan.key_actions_target.exists():
            raise FileExistsError(f"Canonical Key Actions file appeared before apply: {plan.key_actions_target}")
...
def print_plan(plan: MigrationPlan) -> None:
    print("DeepCross canonical naming migration")
    print("=" * 36)
    if plan.errors:
        print("\nVALIDATION ERRORS")
    ...
    for pair in plan.pairs:
        print(f"  [DeepCross #{pair.clip_id}, row {pair.summary_row}]")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Plan or apply the Crosses25+ -> DeepCross canonical naming migration. Dry-run is the default."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 1
    print("\nAPPLY COMPLETE: DeepCross tracking paths, Key Actions source, and DatasetSummary were updated.")
    return 0
