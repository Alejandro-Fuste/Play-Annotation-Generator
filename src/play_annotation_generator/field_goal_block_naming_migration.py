...
LEGACY_NAME = "FieldGoalBlocks"
CANONICAL_NAME = "FieldGoalBlock"
TRACKING_PATTERN = re.compile(r"^FieldGoalBlocks_(?P<id>\d+)_cvat_mot\.zip$")
SUMMARY_PATTERN = re.compile(r"^FieldGoalBlocks_(?P<id>\d+)\.mp4$")
...
def build_plan(tracking_root: Path, dataset_summary: Path, key_actions_root: Path) -> MigrationPlan:
    ...
    if not source_folder.is_dir():
        errors.append(f"[FieldGoalBlock] legacy tracking folder not found: {source_folder}")
    if target_folder.exists():
        errors.append(f"[FieldGoalBlock] canonical tracking folder already exists: {target_folder}")
    if not key_actions_file.is_file():
        errors.append(f"[FieldGoalBlock] canonical Key Actions file not found: {key_actions_file}")
    if errors:
        return MigrationPlan(source_folder, target_folder, key_actions_file, (), tuple(errors))
    ...
    for path in sorted(source_folder.iterdir(), key=lambda p: p.name):
        ...
        if clip_id in tracking_by_id:
            errors.append(f"[FieldGoalBlock] duplicate legacy tracking ID {clip_id}")
        else:
            tracking_by_id[clip_id] = path
    if unexpected:
        errors.append(f"[FieldGoalBlock] unexpected entries in legacy tracking folder: {unexpected}")
    ...
    for row_number, output_file in summary_rows:
        ...
        if clip_id in summary_by_id:
            errors.append(f"[FieldGoalBlock] duplicate legacy DatasetSummary ID {clip_id}")
        else:
    ...
    if not tracking_ids:
        errors.append("[FieldGoalBlock] no legacy FieldGoalBlocks tracking files were found")
    if not summary_ids:
        errors.append("[FieldGoalBlock] no legacy FieldGoalBlocks DatasetSummary rows were found")

    if tracking_ids != summary_ids:
        only_tracking = sorted(tracking_ids - summary_ids)
        only_summary = sorted(summary_ids - tracking_ids)
        if only_tracking:
            errors.append(f"[FieldGoalBlock] IDs present in tracking but missing from DatasetSummary: {only_tracking}")
        if only_summary:
            errors.append(f"[FieldGoalBlock] IDs present in DatasetSummary but missing from tracking: {only_summary}")
    ...
    for clip_id in sorted(tracking_ids & summary_ids):
        ...
        if all_outputs.get(new_output):
            errors.append(
                f"[FieldGoalBlock] DatasetSummary target collision for ID {clip_id}: "
...
def apply_plan(plan: MigrationPlan, dataset_summary: Path) -> None:
    ...
    fd, temp_name = tempfile.mkstemp(
        prefix=f".{dataset_summary.name}.field-goal-block.", suffix=".tmp", dir=str(dataset_summary.parent)
    )
    os.close(fd)
...
def print_plan(plan: MigrationPlan) -> None:
    print("FieldGoalBlock canonical naming migration")
    print("=" * 40)
    if plan.errors:
        print("\nVALIDATION ERRORS")
    ...
    for pair in plan.pairs:
        print(f"  [FieldGoalBlock #{pair.clip_id}, row {pair.summary_row}]")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Plan or apply FieldGoalBlocks -> FieldGoalBlock naming migration. Dry-run is default.")
    ...
    try:
        ...
        return 1
    print("\nAPPLY COMPLETE: FieldGoalBlock tracking identities and DatasetSummary were updated; Key Actions was unchanged.")
    return 0
